...
public class ProdutoDAO extends DAO <Produto>{
    ...
    @SuppressWarnings("unchecked")
        public List<Produto> buscarPorGrupoAlimentar(int codigoGrupo) throws PersistenciaException{
            ...
            try{
                Query query = this.entityManager.createQuery("SELECT p FROM Produto p WHERE p.grupoAlimentar.codigo = :codgrupo ORDER BY p.nome");
        ...
        public List <Produto>buscarPorParteNome(String nome )throws PersistenciaException{
            List <Produto> listaProduto;
            try{
                Query query = this.entityManager.createQuery("SELECT p FROM Produto p WHERE UPPER (p.nome) LIKE:pNome ORDER BY p.nome");
        ...
        @SuppressWarnings("unchecked")
        public List <Produto>busacarProdutoAbaixoEstoqueMinimo()throws PersistenciaException{
            List <Produto> listaProduto;
            try{
                Query query = this.entityManager.createQuery("SELECT p FROM Produto p WHERE p.estoque <p.estoqueMinimo ORDER BY p.nome");
                listaProduto = query.getResultList();
            }catch(Exception ex){
